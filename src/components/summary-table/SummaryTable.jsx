...
function SummaryTable({ data }) {
  ...
  const currentCritical = data.inIcuCurrently
    ? formatNum(data.inIcuCurrently)
    : "-";
  const totalConfirmedCases = data.positive ? formatNum(data.positive) : "-";
  const totalTested = data.totalTestResults
    ? formatNum(data.totalTestResults)
    : "-";
  const newCases = data.positiveIncrease
    ? `+${formatNum(data.positiveIncrease)}`
    : "-";
  const newDeaths = data.deathIncrease
    ? `+${formatNum(data.deathIncrease)}`
    : "-";
  const dataUpdateDate = data.date
  ...
  return (
    <div>
      <div className={styles.dateUpdatedLabel}>{dataUpdateDate}</div>
